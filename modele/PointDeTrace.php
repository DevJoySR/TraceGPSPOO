...
public function getUneTrace($idTrace)
        ...
        */
    {
    // Requête pour récupérer les données de la trace
    $txt_req = "SELECT id, idUtilisateur, dateHeureDebut, dateheureFin, terminee, ";
    $txt_req .= "dureeSecondes, dureeTexte, distanceKm, denivelePositif, deniveleNegatif, ";
    $txt_req .= "depart, arrivee, vitesseMoyenne, centreLatitude, centreLongitude ";
    ...
    // Créer l'objet Trace avec les données récupérées
    $uneTrace = new Trace(
        $ligne->id,
        $ligne->dateHeureDebut,
        $ligne->dateheureFin,
        $ligne->terminee,
        $ligne->idUtilisateur
    );
    
    // Ajouter les propriétés calculées si elles existent
    if ($ligne->dureeSecondes != null) {
        $uneTrace->setDureeSecondes($ligne->dureeSecondes);
    }
    if ($ligne->dureeTexte != null) {
        $uneTrace->setDureeTexte($ligne->dureeTexte);
    }
    if ($ligne->distanceKm != null) {
        $uneTrace->setDistanceKm($ligne->distanceKm);
    }
    if ($ligne->denivelePositif != null) {
        $uneTrace->setDenivelePositif($ligne->denivelePositif);
    }
    if ($ligne->deniveleNegatif != null) {
        $uneTrace->setDeniveleNegatif($ligne->deniveleNegatif);
    }
    if ($ligne->vitesseMoyenne != null) {
        $uneTrace->setVitesseMoyenne($ligne->vitesseMoyenne);
    }
    if ($ligne->centreLatitude != null && $ligne->centreLongitude != null) {
        $centreParcours = new Point(
            $ligne->centreLatitude,
            $ligne->centreLongitude,
            0
        );
        $uneTrace->setCentre($centreParcours);
    }
